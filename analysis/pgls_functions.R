...
### This function sets up and runs a pgls on all taxa
...
### It expects a dataframe with rank, trait1, and trait2
run_all_taxa_and_by_group_pgls <- function(data,tree,group_list){
  # Build the pgls dataset across all taxa
  all_dat <- build_pgls(data,tree)
...
### This function sets up and runs a residual pgls on all taxa
### It expects a dataframe with rank, trait1, trait2, and indep
...
### It expects a dataframe with rank, trait1, and trait2
run_eco_regime_pgls <- function(data,tree,group_list){
  # Build the pgls dataset across all taxa
  all_dat <- build_pgls(data,tree)
  # Run pgls across all taxa
  all_pgls_out <- pgls.trees(all_dat,tree,"trait2 ~ trait1")
...
run_plotting_pgls <- function(data,tree,name){
  ...
  # Plot results
  #pdf(paste(name,".pdf",sep=""),width=plot_width,height=plot_height,useDingbats=F)
...
run_plotting_resid_pgls <- function(data,tree,name){
  ...
  # Plot the results
  #pdf(paste(name,".pdf",sep=""),width=plot_width,height=plot_height,useDingbats=F)
  #  g1 <- ggplot(data,aes(x = trait1, y = trait2, color = group)) + geom_point() + scale_color_manual(values = mrk) + theme(legend.position = "none")
  #print(g1)
  #dev.off()

  #pdf(paste(name,"_residuals.pdf",sep=""),width=plot_width,height=plot_height,useDingbats=F)
    g2 <- ggplot(all_dat,aes(x = trait1, y = trait2, color = group)) + geom_point() + scale_color_manual(values = mrk) + theme(legend.position = "none")
    return(g2)
  #dev.off()
}
