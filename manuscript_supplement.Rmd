...
set.seed(12345)  # set seed for cobmination
...
\textcolor{red}{For regressions of egg size and ovariole number when accounting for adult body size, we compared the results of our regression analyses to distributions estimated using simulated data under alternative hypotheses. We fit a Brownian motion model to the phylogenetic residuals of egg size and body size (R package geiger}[@harmon2007geiger]\textcolor{red}{, version `r packageVersion('geiger')`), and then used the parameters of this fitted model to simulate new datasets (R package phylolm}[@tung2014linear]\textcolor{red}{, version `r packageVersion('phylolm')`). We performed this resimulation using the datasets of egg size and body length at the family level, and egg size and body weight at the genus level. We simulated 1000 datasets each under two hypotheses: no correlation (slope=0) and a strong negative correlation (slope=-1:1). We performed the regressions as described above and compared the distribution of p-values and slopes to values from regressions on observed data.}

```{r simulated, fig.cap="**\\textcolor{red}{Using simulated data to test alternative hypotheses of evolutionary relationships}** Top row, distributions of p-values over 1000 replicate regressions, dashed black line indicates threshold of 0.01. Bottom row, distribution of estimated slopes between egg size and ovariole number, dashed lines indicate 95% interval of simulated distributions. Left, comparing egg size and ovariole number, accounting for body length at the family level. Right, comparing egg size and ovariole number, accounting for body weight at the genus level. Red=observed values, gray=simulated with no correlation, black=simulated with a -1:1 correlation. n=1000 regressions.", fig.width=7.5, fig.height=5, include=TRUE}
...
load("analysis/pgls_results_Fall2020/run_pgls_simulated_allometry_body_egg_ON_family_-1_image.RData")
...
load("analysis/pgls_results_Fall2020/run_pgls_simulated_allometry_body_egg_ON_family_0_image.RData")
...
load("analysis/pgls_results_Fall2020/run_posterior_pgls_egg_size_ovariole_number.RData")
...
load("analysis/pgls_results_Fall2020/run_pgls_simulated_allometry_gilbert_genus_-1_image.RData")
sim_corr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_corr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Fall2020/run_pgls_simulated_allometry_gilbert_genus_0_image.RData")
sim_nocorr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_nocorr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Fall2020/run_posterior_pgls_egg_size_ovariole_number.RData")
...
geom_vline(xintercept=0.01,colour="black",linetype="dashed") + 
#geom_vline(xintercept=0.05,colour="black",linetype="longdash") + 
ggtitle("egg size vs ov. number, body weight") + 
xlab("p-value") +
#ylab("type") + 
xlim(c(0,1)) +
scale_color_manual(values = c("black","dark gray","red")) +
...
### PGLS TABLE SUBCLADE
load("analysis/pgls_results_Fall2020/run_posterior_pgls_subclade.RData")
...
load("analysis/pgls_results_Fall2020/run_posterior_pgls_body_size.RData")
...
load("analysis/pgls_results_Fall2020/run_posterior_pgls_fecundity.RData")
