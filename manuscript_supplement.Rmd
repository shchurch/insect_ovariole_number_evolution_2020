---
title: Repeated loss of variation in insect ovary morphology highlights the role of development in life-history evolution - Supplementary Methods
author: 
    - Samuel H. Church^1^\*, 
    - Bruno A. S. De Medeiros^1,2^, 
    - Seth Donoughe^1,3^, 
    - Nicole L. Márquez Reyes^4^, 
    - Cassandra G. Extavour^1,5^\*
output:
...
Ovariole number was recorded as either an average with deviations, a range, or a single total value. When multiple types of data were available from a single publication, we recorded only a single type, with priority given to averages over ranges, and to both over single total values. Ovariole number was recorded as the total number of ovarioles per female, summing over both the left and right adult ovaries. When authors reported ovariole number from a single ovary, the total value was calculated by doubling the reported value. When authors described differences between the two ovaries, this information was recorded in an additional notes column.

For records of ovariole number that reported intraspecific variation in ovariole number, we calculated the percent difference as follows: if ovariole number was reported as a range, percent difference was calculated as the $100 * ((max - min) / median)$; if ovariole number was reported as an average with deviations, percent difference was calculated as $100 * ((2 * deviation) / mean)$. When independent observations of ovariole number for a given species were available across multiple published records, we calculated the percent difference as the $100 * ((max - min) / median)$ (Fig. \ref{fig:intraspecific-variation}).

Using this approach, we gathered `r nrow(raw_ON)` records for ovariole number from `r raw_ON %>% distinct(publication) %>% nrow()` publications. A full list of publications is provided in the supplementary file 'ovariole_number_bibliography.pdf'. We matched the scientific names to additional taxonomic information using the software TaxReformer[@TaxReformer] and found additional taxonomic data for `r nrow(matched)` of the `r nrow(raw_ON)` records. We verified that TaxReformer had found a valid match by comparing the originally recorded taxonomic order to the order populated by online databases, and removed `r nrow(raw_ON %>% filter(problem == "taxonomy"))` taxonomic records for which these values did not match. For all subsequent analyses, we also excluded observations made in non-reproductive individuals from eusocial species (workers), as well as two observations which represented significant outliers and could not be validated using additional sources or figures[@su2015testicular;@hernandez2017biology].

```{r intraspecific-variation, fig.cap="**Intraspecific variation in ovariole number.**", fig.width=7.5, fig.height=3, include=TRUE}
# calculate the percent difference reported for each especies
percdiff_within_record <- ON %>% filter(!is.na(deviation_on)) %>% 
    filter(!is.na(species)) %>% #check that species name is present
    mutate(percdiff = ifelse(!is.na(deviation_on),100*((2*deviation_on)/mean_on), #if ovariole number is recorded as an average and deviation
        ifelse(!is.na(median_on),100*((maximum_on - minimum_on)/median_on),NA))) %>% #if it is recorded as a range
    filter(!is.na(percdiff) & percdiff>0)

# get the names of species that have more than one description
ON_name_count <- ON %>% 
    filter(species != "") %>% #check that species name is present
    count(name) %>% 
    filter(n > 1) 

percdiff_across_record <- ON %>% 
    filter(name %in% ON_name_count$name) %>% #get records for multiple species
    group_by(name) %>%  
    summarize(max_on = max(on), min_on = min(on),genus=genus) %>%
    ungroup() %>%
    mutate(percdiff = 100*((max_on - min_on)/(min_on + ((max_on - min_on)/2)))) %>% #calculated percent difference
    filter(!is.na(percdiff) & percdiff>0)

pd1 <- ggplot(percdiff_within_record,aes(x=percdiff)) + geom_histogram(bins=25) + scale_x_log10() + ggtitle("reported variation within observation") + xlab("percent difference")
pd2 <- ggplot(percdiff_across_record,aes(x=percdiff)) + geom_histogram(bins=25) + scale_x_log10() + ggtitle("variation across repeated observations") + xlab("percent difference")
...
For regressions involving body size data that were reported as insect family-level averages, we used the insect phylogeny published in Rainford et. al, 2014[@rainford2014phylogenetic].
...
We combined the data we collected on total ovariole number with existing datasets of egg size and shape[@church2019dataset], insect lifetime fecundity and dry adult body mass[@Gilbert2007;@gilbert2010parental;@gilbert2011insect], average adult body length per insect family[@rainford2016phylogenetic], and several lineage-specific measures of adult body size[@iwata1966large;@Iwata1966a;@waloff1954number;@starmer2003phylogenetic;@reinhardt2005low].

Ovariole number and egg size[@church2019insect] data were combined by matching records across datasets for the same insect species (Fig 2a). When multiple records existed for a given species, the dataset was randomly shuffled and a single matching record was selected. This variation across records for the same species was accounted for in regressions by reshuffling and matching records at each iteration of the analysis.  We also matched records for insects in the same genus following the same reshuffling method, which allowed us to test whether results were robust with a larger sample size when an exact species match was not available (Fig. \ref{fig:PGLS-egg-ON-genus}).

Average adult body length per insect family[@rainford2016phylogenetic] was matched to the average ovariole number for the corresponding families (Fig. \ref{fig:PGLS-family-length}). The Rainford et al, 2016[@rainford2016phylogenetic] dataset contains a small number of average adult body lengths at the order level (e.g. Strepsiptera), which were matched to their equivalent group in the ovariole number dataset. To test the effect of uncertainty in the estimated average ovariole number on our results, the dataset for each family was downsampled by half at each iteration of the regression analysis.

Ovariole number, egg volume, lifetime fecundity, and adult body mass[@Gilbert2007;@gilbert2010parental;@gilbert2011insect] were combined by matching records at the species level and genus level, using the same method as described above (Figs. 2b, 3, and \ref{fig:PGLS-Gilbert-name}). We excluded one value from this dataset which appeared to include a typographical error for lifetime fecundity (Hymenoptera: Trichogrammatidae, lifetime fecundity recorded as 0.1[@Gilbert2007]).
...
# combine datasets, using all data and randomly shuffled database entries
downsample_factor <- 1
fam_count_threshold <- 1

set.seed(12345)  # set seed for combination
source("analysis/combine_body_egg_datasets.R")

# use the maximum clade credibility tree
tree <- genus_mcc_tree
```

```{r PGLS-egg-ON}
### PGLS egg volume vs ON, species level pgls
pgls_results_egg_ON_name <- run_all_taxa_pgls(egg_ON_name %>% 
    select(genus,logvol,logon,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logvol) ,
    tree)

egg_ON_name_plot <- egg_ON_name %>% 
    select(genus,vol,on,group) %>% 
    rename(rank = genus, trait1 = on, trait2 = vol) 
...
g1 <- ggplot(egg_ON_name_plot,aes(x = trait1, y = trait2, color = group)) +
    geom_point() +
...
egg_ON_genus_pgls <- egg_ON_genus %>% 
    select(genus,logvol,logon,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logvol) 

pgls_results_egg_ON_genus <- run_all_taxa_pgls(egg_ON_genus_pgls,tree)
...
```{r PGLS-egg-ON-genus-noneusocial, fig.cap="**Egg volume vs ovariole number, matching records at the genus level, excluding records from families with eusocial insects.**", fig.width=7.5, fig.height=5, include=TRUE}
### PGLS on vs egg volume by insect genus, excluding eusocial
eusocial_family <- ON %>% filter(grepl("queen",condition)) %>% pull(family) %>% unique()

egg_ON_genus_noneusocial_pgls <- egg_ON_genus %>% 
    filter(!(family %in% eusocial_family)) %>% 
    select(genus,logvol,logon,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logvol) 

pgls_results_egg_ON_genus_noneusocial <- run_all_taxa_pgls(egg_ON_genus_noneusocial_pgls,tree)

run_plotting_pgls(egg_ON_genus_noneusocial_pgls,tree,"egg_ON_genus_noneusocial") +
    xlab(bquote('ovariole number,'~log[10])) +
    ylab(bquote('egg volume,'~mm^3~','~log[10]))
```

```{r PGLS-egg-ON-weight-genus}
### Gilbert egg vs ON, ind body weight genus pgls
gilbert_genus_pgls <- egg_ON_gilbert_genus %>% 
    select(genus,logvol,logon,logweight,group) %>% 
    mutate(rank = genus, trait1 = logon, trait2= logvol, indep = logweight) 

pgls_results_gilbert_genus <- run_all_taxa_resid_pgls(gilbert_genus_pgls,tree)
...
run_plotting_resid_pgls(gilbert_genus_pgls,tree,"gilbert_genus") + 
    xlab("residual of ovariole number") + 
    ylab("residual of egg volume")
invisible(dev.off())
```

```{r PGLS-Gilbert-name, fig.cap="**Egg volume vs ovariole number, phylogenetic residuals to dry adult body mass, matching records at the species level.**", fig.width=3, fig.height=3, include=TRUE}
### Gilbert egg vs ON, ind body weight name pgls
egg_ON_gilbert_name_pgls <- egg_ON_gilbert_name %>% 
    select(genus,logvol,logon,logweight,group) %>% 
    mutate(rank = genus, trait1 = logon, trait2= logvol, indep = logweight) 

pgls_results_gilbert_name <- run_all_taxa_resid_pgls(egg_ON_gilbert_name_pgls,tree)

### Gilbert body weight, species level pgls
run_plotting_resid_pgls(egg_ON_gilbert_name_pgls,tree,"gilbert_name") + 
    xlab("residual of ovariole number") + 
    ylab("residual of egg volume")
```

```{r PGLS-family-length, fig.cap="**Family average egg volume vs ovariole number, phylogenetic residuals to adult body length.**",  fig.width=3, fig.height=3, include=TRUE}
### PGLS egg vs ON, independent body size
body_egg_ON_family_pgls <- body_egg_ON_full %>% 
    select(rank,logvol,logon,logbody,group) %>% 
    rename(trait1 = logon, trait2= logvol, indep = logbody) 

pgls_results_body_egg_ON_family <- run_all_taxa_resid_pgls(body_egg_ON_family_pgls,fam_tree)

run_plotting_resid_pgls(body_egg_ON_family_pgls,fam_tree,"body_egg_ON_family") + 
    xlab("residual of average ovariole number") +
    ylab("residual of average egg volume")
```

We ran each Phylogenetic Generalized Least Squares (PGLS) regression over the Maximum Clade Credibilty (MCC) tree. For each regression, we also repeated each PGLS analysis 1000 times, accounting for phylogenetic and phenotypic uncertainty, using the R packages ape (version `r packageVersion('ape')`)[@paradis2004ape] and nlme (version `r packageVersion('nlme')`)[@pinheiro2014r]. In these analyses we used a Brownian Motion based covariance matrix for traits. 

For regressions using data matched across species or genera, we reshuffled and matched records at each iteration to account for variation across records for the same taxon. For regressions on family-level average data, we recalculated the average ovariole number per insect family, downsampling the representation for each family by half. No posterior distribution was available with the previously published family level phylogeny[@rainford2014phylogenetic].

To account for body size, we calculated the phylogenetic residuals[@revell2009size] of each trait to body size, and then compared the evolution of these residuals using a PGLS regression.

For regressions of egg size and ovariole number when accounting for adult body size, we compared the results of our regression analyses to distributions estimated using simulated data under alternative hypotheses. We fit a Brownian motion model to the phylogenetic residuals of egg size and body size (R package geiger, version `r packageVersion('geiger')`)[@harmon2007geiger], and then used the parameters of this fitted model to simulate new datasets (R package phylolm, version `r packageVersion('phylolm')`)[@tung2014linear]. We performed this resimulation using the datasets of egg size and body length at the family level, and egg size and body mass at the genus level. We simulated 1000 datasets each under two hypotheses: no correlation (slope=0) and a strong negative correlation (slope=-1). We performed the regressions as described above and compared the distribution of p-values and slopes to values from regressions on observed data.

```{r simulated, fig.cap="**Using simulated data to test alternative hypotheses of evolutionary relationships.** Top row, distributions of p-values over 1000 replicate regressions, dashed black line indicates threshold of 0.01. Bottom row, distribution of estimated slopes between egg size and ovariole number, dashed lines indicate 95% interval of simulated distributions. Left, comparing egg size and ovariole number, accounting for body length at the family level. Right, comparing egg size and ovariole number, accounting for body mass at the genus level. The slope of regressions on observed egg size and ovariole number (red) is more negative than we would expect to observe by chance, as assessed by comparing to data simulated with no evolutionary correlation (gray), but also not within the range that would be expected under an strong negative correlation (black). Red=observed values, gray=simulated with no correlation, black=simulated with a -1:1 correlation. n=1000 regressions.", fig.width=7.5, fig.height=5, include=TRUE}
get_tstat <- function(sum,n) {
    tstat <- as.numeric(unname(sum[[n]]$coefficients[[2]]))
    return(tstat)
}

get_pval <- function(sum,n) {
    pval <- as.numeric(unname(sum[[n]]$tTable[[8]]))
    return(pval)
}

load("analysis/pgls_results_Jan2021/run_pgls_simulated_allometry_body_egg_ON_family_-1_image.RData")
sim_corr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_corr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Jan2021/run_pgls_simulated_allometry_body_egg_ON_family_0_image.RData")
sim_nocorr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_nocorr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Jan2021/run_posterior_pgls_egg_size_ovariole_number.RData")
obs_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=3)
obs_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=3)

body_egg_ON_family_tstat_dist <- rbind(tibble(type = "simulated",
    relationship = "-1:1 correlation",
    tstat=sim_corr_tstat_dist),
tibble(type = "simulated",
    relationship = "no correlation",
    tstat=sim_nocorr_tstat_dist),
tibble(type = "observed",
    relationship = "observed",
    tstat=obs_tstat_dist))

body_egg_ON_family_pval_dist <- rbind(tibble(type = "simulated",
    relationship = "-1:1 correlation",
    pval=sim_corr_pval_dist),
tibble(type = "simulated",
    relationship = "no correlation",
    pval=sim_nocorr_pval_dist),
tibble(type = "observed",
    relationship = "observed",
    pval=obs_pval_dist))


load("analysis/pgls_results_Jan2021/run_pgls_simulated_allometry_gilbert_genus_-1_image.RData")
sim_corr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_corr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Jan2021/run_pgls_simulated_allometry_gilbert_genus_0_image.RData")
sim_nocorr_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=1)
sim_nocorr_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=1)
load("analysis/pgls_results_Jan2021/run_posterior_pgls_egg_size_ovariole_number.RData")
obs_tstat_dist <- sapply(allometry_distribution_raw,get_tstat,n=5)
obs_pval_dist <- sapply(allometry_distribution_raw,get_pval,n=5)

gilbert_genus_pval_dist <- rbind(tibble(type = "simulated",
    relationship = "-1:1 correlation",
    pval=sim_corr_pval_dist),
tibble(type = "simulated",
    relationship = "no correlation",
...
gilbert_genus_tstat_dist <- rbind(tibble(type = "simulated",
    relationship = "-1:1 correlation",
    tstat=sim_corr_tstat_dist),
tibble(type = "simulated",
    relationship = "no correlation",
...
t1 <- ggplot(body_egg_ON_family_tstat_dist,aes(x=tstat,y=relationship,color=relationship)) + geom_jitter(size=0.2)  + 
geom_vline(xintercept=quantile(body_egg_ON_family_tstat_dist %>% filter(relationship == "-1:1 correlation") %>% pull(tstat),0.05),colour="black",linetype="longdash") + 
geom_vline(xintercept=quantile(body_egg_ON_family_tstat_dist %>% filter(relationship == "-1:1 correlation") %>% pull(tstat),0.95),colour="black",linetype="longdash") + 
geom_vline(xintercept=quantile(body_egg_ON_family_tstat_dist %>% filter(relationship == "no correlation") %>% pull(tstat),0.05),colour="dark gray",linetype="longdash") + 
geom_vline(xintercept=quantile(body_egg_ON_family_tstat_dist %>% filter(relationship == "no correlation") %>% pull(tstat),0.95),colour="dark gray",linetype="longdash") + 
#ggtitle("size vs number, body length, family")  + 
xlab("test statistic, slope") +
#ylab("type") + 
xlim(c(-2,2)) +
scale_color_manual(values = c("black","dark gray","red")) +
theme(legend.position="none",
    axis.title.y=element_blank())

t2 <- ggplot(gilbert_genus_tstat_dist,aes(x=tstat,y=relationship,color=relationship)) + geom_jitter(size=0.2)  + 
geom_vline(xintercept=quantile(gilbert_genus_tstat_dist %>% filter(relationship == "-1:1 correlation") %>% pull(tstat),0.05),colour="black",linetype="longdash") + 
geom_vline(xintercept=quantile(gilbert_genus_tstat_dist %>% filter(relationship == "-1:1 correlation") %>% pull(tstat),0.95),colour="black",linetype="longdash") + 
geom_vline(xintercept=quantile(gilbert_genus_tstat_dist %>% filter(relationship == "no correlation") %>% pull(tstat),0.05),colour="dark gray",linetype="longdash") + 
geom_vline(xintercept=quantile(gilbert_genus_tstat_dist %>% filter(relationship == "no correlation") %>% pull(tstat),0.95),colour="dark gray",linetype="longdash") + 
#ggtitle("size vs number, body weight, genus") + 
xlab("test statistic, slope") +
#ylab("type") + 
xlim(c(-2,2)) +
scale_color_manual(values = c("black","dark gray","red")) +
...
p1 <- ggplot(body_egg_ON_family_pval_dist,aes(x=pval,y=relationship,color=relationship)) + geom_jitter(size=0.2)  + 
geom_vline(xintercept=0.01,colour="black",linetype="dashed") + 
#geom_vline(xintercept=0.05,colour="black",linetype="longdash") + 
ggtitle("egg size vs ov. number, body length")  + 
xlab("p-value") +
#ylab("type") + 
xlim(c(0,1)) +
scale_color_manual(values = c("black","dark gray","red")) +
...
p2 <- ggplot(gilbert_genus_pval_dist,aes(x=pval,y=relationship,color=relationship)) + geom_jitter(size=0.2)  + 
geom_vline(xintercept=0.01,colour="black",linetype="dashed") + 
#geom_vline(xintercept=0.05,colour="black",linetype="longdash") + 
ggtitle("egg size vs ov. number, body mass") + 
xlab("p-value") +
#ylab("type") + 
xlim(c(0,1)) +
scale_color_manual(values = c("black","dark gray","red")) +
...
COL1_body_egg_ON_genus_pgls <- COL1_body_egg_ON_genus %>% 
    select(genus,logvol,logon,logbody,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logvol, indep = logbody) 

pgls_results_COL1 <- run_all_taxa_resid_pgls(COL1_body_egg_ON_genus_pgls,tree)

theme_update(plot.title = element_text(hjust = 0.5))
col1 <- run_plotting_resid_pgls(COL1_body_egg_ON_genus_pgls,tree,"COL1") + 
    xlab("residual of ovariole number") +
    ylab("residual of egg volume") + 
    ggtitle("Coleoptera: Curculionoidea")

### Apoidea ON, egg, indep body pgls
HYM_body_egg_ON_genus_pgls <- HYM_body_egg_ON_genus %>% 
    select(genus,logvol,logon,logbody,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logvol, indep = logbody) 

pgls_results_HYM <- run_all_taxa_resid_pgls(HYM_body_egg_ON_genus_pgls,tree)

hym1 <- run_plotting_resid_pgls(HYM_body_egg_ON_genus_pgls,tree,"HYM") + xlab("residual of ovariole number") +
ylab(element_blank()) + ggtitle("Hymenoptera")

grid.arrange(col1,hym1,ncol=2)
```

```{r PGLS-body-plot, fig.cap="**Adult body size vs ovariole number.** Adult body mass, matching records at the genus level (left), and family-level average adult body length (right).", fig.width=6, fig.height=3, include=TRUE}
### body length vs ON, family level
ON_body_family_pgls <- body_egg_ON_full %>% 
    select(rank,logon,logbody,group) %>% 
    rename(trait1 = logon, trait2 = logbody) 

pgls_results_body_ON_family <- run_all_taxa_pgls(ON_body_family_pgls,fam_tree)

family_body <- run_plotting_pgls(ON_body_family_pgls,fam_tree,"body_egg_ON_family") + 
    xlab(bquote('average ovariole number,'~log[10])) +
    ylab(bquote('average adult body length (mm),'~log[10]))

### Gilbert body weight vs ON, species level
pgls_results_body_gilbert_name <- run_all_taxa_pgls(egg_ON_gilbert_name %>% 
    select(genus,logon,logweight,group) %>% 
    mutate(rank = genus, trait1 = logon, trait2 = logweight) ,
    tree)

### Gilbert body weight, genus level
ON_body_gilbert_genus_pgls <- egg_ON_gilbert_genus %>% 
    select(genus,logon,logweight,group) %>% 
    mutate(rank = genus, trait1 = logon, trait2 = logweight) 

pgls_results_body_gilbert_genus <- run_all_taxa_pgls(ON_body_gilbert_genus_pgls,tree)

gilbert_body <- run_plotting_pgls(ON_body_gilbert_genus_pgls,tree,"body_gilbert") + xlab(bquote('ovariole number,'~log[10])) +
ylab(bquote('dry adult body mass (g),'~log[10]))

### Curculionoidea body vs ON
pgls_results_body_COL1 <- run_all_taxa_pgls(COL1_body_egg_ON_genus %>% 
    select(genus,logon,logbody,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logbody) ,
    tree)

### Orthoptera body vs ON
pgls_results_body_ORT <- run_all_taxa_pgls(ORT_body_egg_ON_genus %>% 
    select(genus,logon,logbody,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logbody) ,
    tree)

### Apoidea body vs ON
pgls_results_body_HYM <- run_all_taxa_pgls(HYM_body_egg_ON_genus %>% 
    select(genus,logon,logbody,group) %>% 
    rename(rank = genus, trait1 = logon, trait2 = logbody) ,
    tree)

### Drosophilidae body vs ON
pgls_results_body_DIP <- run_all_taxa_pgls (DIP_body_egg_ON_name %>% 
...
fecundity_name_pgls <-  ON_gilbert_name %>% 
    select(genus,logfecundity,logon,group) %>% 
    mutate(rank = genus, trait1 = logon, trait2= logfecundity) 

pgls_results_fecundity_name <- run_all_taxa_pgls(fecundity_name_pgls,tree)

run_plotting_pgls(fecundity_name_pgls,tree,"fecundity_name") + 
    xlab(bquote("ovariole number,"~log[10])) +
...
fecundity_genus_pgls <-  ON_gilbert_genus %>% 
    select(genus,fecundity,on,group) %>% 
    mutate(rank = genus, trait1 = on, trait2= fecundity)  

pgls_results_fecundity_genus <- run_all_taxa_pgls(fecundity_genus_pgls,tree)
...
```{r PGLS-posterior-egg-ON, include=TRUE}
# retrieve results from over the posterior distribution
get_pgls_table <- function(dist) {
    table <- round(data.frame(
    slope_min = min(sapply(dist,function(x) {x$coefficients[[2]]})),
    slope_max = max(sapply(dist,function(x) {x$coefficients[[2]]})),
    pval_sig = sum(sapply(dist,function(x) {x$tTable[[8]] < 0.01})),
    taxa = min(sapply(dist,function(x) {x$dims$N}))),
    4)
    return(table)
}
...
### PGLS TABLE EGG SIZE OVARIOLE NUMBER
load("analysis/pgls_results_Jan2021/run_posterior_pgls_egg_size_ovariole_number.RData")

egg_ON_posterior_results <- list(allometry_egg_ON_name,
    allometry_egg_ON_genus,
    allometry_gilbert_name,
    allometry_gilbert_genus,
    allometry_body_egg_ON_family)

egg_ON_pgls_results <- list(pgls_results_egg_ON_name,
    pgls_results_egg_ON_genus,
    pgls_results_gilbert_name,
    pgls_results_gilbert_genus,
    pgls_results_body_egg_ON_family)

egg_ON_pgls_analysis_names <- c("ovariole number vs egg volume",
    "ovariole number vs egg volume",
    "ovariole number vs egg volume, residuals to body mass",
    "ovariole number vs egg volume, residuals to body mass",
    "ovariole number vs egg volume, residuals to body length")

egg_ON_taxonomic_levels <- c("species","genus","species","genus","family average")

egg_ON_table <- as.data.frame(bind_rows(lapply(egg_ON_posterior_results,get_pgls_table))) %>% mutate(analysis = egg_ON_pgls_analysis_names,
        "data match" = egg_ON_taxonomic_levels,
        "slope" = paste(sprintf("%.3f",slope_min),sprintf("%.3f",slope_max),sep=sp),
        "MCC p-value" = gsub("0.000","<0.001",sprintf("%.3f",sapply(egg_ON_pgls_results,function(x){x$tTable[[8]]}))),
        "num. sig. / 1000" = pval_sig,
        "taxa" = taxa)
knitr::kable(egg_ON_table %>% select(c("analysis","data match","slope","MCC p-value","num. sig. / 1000","taxa")),
        "latex",
        align=rep('l', 6),
        caption="Results of PGLS analysis of ovariole number and egg size across a posterior distribution. The 'data match' columns describes how observations were matched across datasets, e.g. matching records for the same species, genera, or using family-level averages.", booktabs = T, linesep = "") %>% 
...
```{r PGLS-posterior-clade, include=TRUE}
### PGLS TABLE SUBCLADE
load("analysis/pgls_results_Jan2021/run_posterior_pgls_subclade.RData")

clade_posterior_results <- list(allometry_DIP,
    allometry_ORT,
    allometry_COL1,
    allometry_HYM)

clade_pgls_results <- list(pgls_results_DIP,
    pgls_results_ORT,
    pgls_results_COL1,
    pgls_results_HYM)

clade_pgls_analysis_names <- c("Drosophilidae ovariole number vs egg volume, residuals to thorax length",
...
clade_taxonomic_levels <- c("species","genus","genus","genus")

clade_table <- as.data.frame(bind_rows(lapply(clade_posterior_results,get_pgls_table))) %>% mutate(analysis = clade_pgls_analysis_names,
        "data match" = clade_taxonomic_levels,
        "slope" = paste(sprintf("%.3f",slope_min),sprintf("%.3f",slope_max),sep=sp),
        "MCC p-value" = gsub("0.000","<0.001",sprintf("%.3f",sapply(clade_pgls_results,function(x){x$tTable[[8]]}))),
        "num. sig. / 1000" = pval_sig,
        "taxa" = taxa)
knitr::kable(clade_table %>% select(c("analysis","data match","slope","MCC p-value","num. sig. / 1000","taxa")),
        "latex",
        align=rep('l', 6),
        caption="Results of PGLS analysis of ovariole number and egg size across a posterior distribution.", booktabs = T, linesep = "") %>% 
    kableExtra::kable_styling(latex_options = "striped") %>%
    kableExtra::column_spec(1, width = "16em") %>%
    kableExtra::kable_styling(latex_options = "hold_position")  %>%
    kableExtra::kable_styling(font_size = 8) 
...
```{r PGLS-posterior-body, include=TRUE}
### PGLS TABLE BODY SIZE
load("analysis/pgls_results_Jan2021/run_posterior_pgls_body_size.RData")

body_posterior_results <- list(allometry_body_gilbert_name,
    allometry_body_gilbert_genus,
    allometry_body_ON_family,
    allometry_body_DIP,
    allometry_body_ORT,
    allometry_body_COL1,
    allometry_body_HYM)

body_pgls_results <- list(pgls_results_body_gilbert_name,
    pgls_results_body_gilbert_genus,
    pgls_results_body_ON_family,
    pgls_results_body_DIP,
    pgls_results_body_ORT,
    pgls_results_body_COL1,
    pgls_results_body_HYM)

body_pgls_analysis_names <- c("ovariole number vs body length",
    "ovariole number vs body mass",
    "ovariole number vs body mass",
...
body_taxonomic_levels <- c("species","genus","family average","species","genus","genus","genus")


body_table <- as.data.frame(bind_rows(lapply(body_posterior_results,get_pgls_table))) %>% mutate(analysis = body_pgls_analysis_names,
        "data match" = body_taxonomic_levels,
        "slope" = paste(sprintf("%.3f",slope_min),sprintf("%.3f",slope_max),sep=sp),
        "MCC p-value" = gsub("0.000","<0.001",sprintf("%.3f",sapply(body_pgls_results,function(x){x$tTable[[8]]}))),
        "num. sig. / 1000" = pval_sig,
        "taxa" = taxa)
knitr::kable(body_table %>% select(c("analysis","data match","slope","MCC p-value","num. sig. / 1000","taxa")),
        "latex",
        align=rep('l', 6),
        caption="Results of PGLS analysis of ovariole number and body size across a posterior distribution.", booktabs = T, linesep = "") %>% 
    kableExtra::kable_styling(latex_options = "striped") %>%
    kableExtra::column_spec(1, width = "16em") %>%
    kableExtra::kable_styling(latex_options = "hold_position")  %>%
    kableExtra::kable_styling(font_size = 8) 
...
```{r PGLS-posterior-fecundity, include=TRUE}
### PGLS TABLE FECUNDITY
load("analysis/pgls_results_Jan2021/run_posterior_pgls_fecundity.RData")

fecundity_posterior_results <- list(allometry_fecundity_name,
    allometry_fecundity_genus)

fecundity_pgls_results <- list(pgls_results_fecundity_name,
    pgls_results_fecundity_genus)

fecundity_pgls_analysis_names <- c("ovariole number vs lifetime fecundity",
    "ovariole number vs lifetime fecundity")

fecundity_taxonomic_levels <- c("species","genus")

fecundity_table <- as.data.frame(bind_rows(lapply(fecundity_posterior_results,get_pgls_table))) %>% mutate(analysis = fecundity_pgls_analysis_names,
        "data match" = fecundity_taxonomic_levels,
        "slope" = paste(sprintf("%.3f",slope_min),sprintf("%.3f",slope_max),sep=sp),
        "MCC p-value" = gsub("0.000","<0.001",sprintf("%.3f",sapply(fecundity_pgls_results,function(x){x$tTable[[8]]}))),
        "num. sig. / 1000" = pval_sig,
        "taxa" = taxa)
knitr::kable(fecundity_table %>% select(c("analysis","data match","slope","MCC p-value","num. sig. / 1000","taxa")),
        "latex",
        caption="Results of PGLS analysis of ovariole number and fecundity across a posterior distribution.", booktabs = T, linesep = "") %>% 
    kableExtra::kable_styling(latex_options = "striped") %>%
    kableExtra::column_spec(1, width = "16em") %>%
    kableExtra::kable_styling(latex_options = "hold_position")  %>%
    kableExtra::kable_styling(font_size = 8) 
...
Using these 106 records for ovary type, we reconstructed the ancestral state at each node of the published phylogeny of `r length(pp$phy$tip.label)` insect genera using an equal-rates model that allows for missing data (Fig. \ref{fig:reconstruct-ovary}, R package corHMM, version 1.22[@beaulieu2013identifying] function rayDISC, node.states = 'marginal').
...
We evaluated the fit of a Brownian Motion (BM) model for ovariole number evolution using the R package arbutus (version `r packageVersion('arbutus')`[@pennell2015model], Fig. \ref{fig:arbutus}). In this approach, a BM model is fit to the data (R package geiger, version `r packageVersion('geiger')`)[@harmon2007geiger], and the resulting parameters of the model are used to simulate 1000 new datasets. Six statistical parameters are used to compare the phylogenetic contrasts of the observed data to the simulated data, and their interpretations are as follows[@pennell2015model]:
...
#set.seed(12345)  # set seed for BAMM
...
### Assess convergence
mcmcout <- read.csv("analysis/BAMM_results_Jan2021/bamm_mcmc_out.txt", header=T) 
...
The best configuration of rate shift regimes shows multiple independent clades with very low rates of evolution (Fig. \ref{fig:BAMM-analysis-results}). Visualizing the distribution of mean rates along branches revealed a discontinuous distribution, with one peak at a moderate rate of evolution and several clusters at extremely low rates, separated from the first peak by over six orders of magnitude (Fig. \ref{fig:BAMM-anlaysis-mean-rates}). We used this visualization to establish a threshold (`r lowrate`) for assigning a binary rate regime to each node in the phylogeny, categorizing them as above (variable) or below (invariant) a threshold that separates these two peaks.
...
```{r BAMM-egg-setup}
#set.seed(12345)  # set seed for egg BAMM
...
```{r BAMM-egg-read}
egg_mcmcout <- read.csv("analysis/BAMM_results_Jan2021/egg_bamm_mcmc_out.txt", header=T)
...
egg_bamm_tree <- ladderize(read.tree(file="analysis/BAMM_results_Jan2021/egg_bamm_tree.tre"))
egg_edata <- getEventData(egg_bamm_tree, eventdata = paste("analysis/BAMM_results_Jan2021/egg_bamm_event_data.txt",sep=""), burnin=egg_burnin, type="trait")
```
...
We assessed the rate of egg volume diversification across insects using the same method of assessing rate heterogeneity as described in Section \ref{sec:BAMM} (Fig. \ref{fig:BAMM-egg-plot}). This analysis converged in $7*10^9$ generations (the effective size for the number of shifts and log-likelihood were >200, `r round(egg_es_nshifts,2)` and `r round(egg_es_logl,2)` respectively). We compared the correlation between the rates of trait diversification for ovariole number and egg volume by matching the mean rate predicted for each insect genus (the tips of the phylogeny from the BAMM analyses, Fig. \ref{fig:combine-rates}).
